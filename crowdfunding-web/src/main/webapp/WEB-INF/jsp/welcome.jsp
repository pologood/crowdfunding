...
<script type="text/javascript" src="<%=request.getContextPath()%>/static/Guriddo_jqGrid_JS_4.7.1/js/jquery.jqGrid.src.js"></script>

<script type="text/javascript">


function ajaxResponseHandler(json,alertTrueMessage,successHandler,failHandler){
	if(json){
        if(json.result == false){
	        if(json.message)alert(json.message);
	        if(failHandler)failHandler(json);
        }
        if(json.result == true){
		    if(alertTrueMessage == true){
		    	if(json.message)alert(json.message);
		    }
...
function deleteUser(cellvalue){
    $.ajax({
        ...
        success: function(retData){
            ajaxResponseHandler(retData,true,function(retData){
                alert("删除成功");
...
<body>
YOUKU Crowdfunding <a href="<%=request.getContextPath()%>/index/j_spring_security_logout" >退出</a><br/>
<!-- 
